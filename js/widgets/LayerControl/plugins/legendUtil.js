define([
    ...
    return {
        ...
        _image: function (legend, layerId, layer) {
            ...
            return '<img src="' + src + '" class="' +  layer.id + '-layerLegendImage" ' +
                    'style="width:' + legend.width + 'px; height:' + legend.height + 'px; opacity:' + layer.opacity + ';"/>';
        },
        ////////////////////////////////////////
        ...
        _createDynamicSublayerLegend: function (layer, expandNode, r) {
            array.forEach(r.layers, function (_layer) {
                ...
                array.forEach(_layer.legend, function (legend) {
                    ...
                    var row = domConst.create('tr', {}, table, 'last'),
                        symbol = domConst.create('td', {
                            ...
                            innerHTML: this._image(legend, layerId, layer)
                        }, row, 'first');
                    // create label td
                    domConst.create('td', {
                ...
                if (layer.layerInfos.length > 1) {
                    var sublayer = registry.byId(layer.id + '-' + _layer.layerId + '-sublayer-control');
                    if (sublayer){
                        var sublayerExpandNode = sublayer.expandNode;
        ...
        _dynamicSublayerLegendError: function (layer, expandNode, e) {
            ...
            if (expandNode.legendErrorCallback){
                expandNode.legendErrorCallback();
            }
